...
class NFCReaderGUI(QMainWindow):
    ...
    # Printing runs on a worker thread; this carries failures back to the GUI thread.
    printer_failed = pyqtSignal(str)
    ...
    def __init__(self):
        ...
        initial_config = load_device_config()
        self._set_classroom_attributes(initial_config, refresh_prompt=False)
        
        # Initialize GPIO for LED control
        self.setup_gpio()
        
        # Initialize Online-First database (Firebase primary, SQLite backup when offline)
        self.db = OnlineFirstDatabase(
            connectivity_check_interval=30,
            classroom_context=self.device_config
        )
        ...
        self.printer_failed.connect(self.show_error_message)
    ...
    def _set_classroom_attributes(self, config, refresh_prompt=True):
        ...
        self.classroom_id = (self.device_config.get('classroom_id') or '').strip()
        self.classroom_label = (self.device_config.get('classroom_label') or '').strip()
        self.teacher_name = (self.device_config.get('teacher_name') or '').strip()

        if refresh_prompt and hasattr(self, 'prompt'):
            self.refresh_classroom_prompt()

        if refresh_prompt and hasattr(self, 'settings_overlay'):
            self.settings_overlay.populate_classroom_fields()
    ...
    def save_classroom_settings(self, classroom_id, classroom_label, teacher_name):
        ...
        updated_config = update_device_config(
            classroom_id=classroom_id.strip(),
            classroom_label=classroom_label.strip(),
            teacher_name=teacher_name.strip()
        )
        self._set_classroom_attributes(updated_config)
        if hasattr(self, 'db'):
            try:
                self.db.update_classroom_context(updated_config)
    ...
    def process_bathroom_entry(self, student_id=None, nfc_uid=None):
        ...
        if not is_on_break and self._is_bathroom_restricted(datetime.now()):
            self.show_error_message(
                "You can't use the bathroom during the first or last 10 minutes of class."
            )
            return
        ...
        if is_on_break:
            ...
            if success:
                ended_existing_break = "previous break ended" in str(message).lower()
                if ended_existing_break:
                    self.show_prompt_message("Bathroom break ended!")
                else:
        ...
        def work():
            try:
                ...
                if not printed:
                    ...
                    print(f"[PRINTER] Pass was not printed: {reason}")
                    self.printer_failed.emit(f"Pass did not print: {reason}")
            except Exception as e:
                print(f"[PRINTER] Background print failed: {e}")
                self.printer_failed.emit(f"Pass did not print: {e}")
    ...
    def process_nurse_entry(self, student_id=None, nfc_uid=None):
        ...
        if is_on_nurse_visit:
            ...
            success, message = self.db.start_nurse_visit(**visit_kwargs)
            if success:
    ...
    def process_water_entry(self, student_id=None, nfc_uid=None):
        ...
        if is_at_water:
            ...
            success, message = self.db.start_water_visit(**visit_kwargs)
            if success:
                ended_existing_visit = "previous water visit ended" in str(message).lower()
                if ended_existing_visit:
                    self.show_prompt_message("Water visit ended!")
                else:
    ...
    def show_prompt_message(self, message, duration=None):
        """Show a temporary prompt; a new message safely replaces its timer."""
        if duration is None:
            duration = self._INFO_MESSAGE_MS
    ...
    def show_error_message(self, message, duration=None):
        """Show a student-facing error long enough to read on the touchscreen."""
        if duration is None:
            duration = self._ERROR_MESSAGE_MS
        self.show_prompt_message(message, duration)
