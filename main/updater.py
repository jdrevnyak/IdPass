...
class UpdateDownloader(QThread):
    ...
    def __init__(self, update_info, deposit_dir=None):
        ...
        if deposit_dir:
            ...
            # Default: parent directory's deposit folder
    ...
    def run(self):
        ...
        try:
            ...
            # Check if there's a main/ subdirectory (should not be in repo but handle it)
            main_subdir = source_dir / "main"
            if main_subdir.exists() and main_subdir.is_dir():
                print(f"[UPDATE] WARNING: Detected main/ subdirectory in release - ignoring it")
            
            self.status_update.emit("Copying files to deposit folder...")
            
            # Copy files from extracted directory to deposit folder
            # Skip hidden files and the main/ subdirectory
            files_copied = 0
            for item in source_dir.iterdir():
                ...
                if item.name == 'main' and item.is_dir():
                    print(f"[UPDATE] Skipping main/ subdirectory (runtime directory, should not be in release)")
                    continue
