...
class FirebaseDatabase:
    ...
    def ping_firestore(self) -> Tuple[bool, str]:
        """Lightweight read to verify Firestore is reachable."""
        try:
            self.db.collection('settings').document('periods').get()
            return True, "Firestore responded"
        except Exception as e:
            return False, str(e)
    ...
    def _period_end_datetime_for_timestamp(self, dt: datetime) -> Optional[datetime]:
        """
        End-of-period datetime (today's date) for the school period whose window contains dt.
        Used to auto-close outings that started during that period when now passes period end.
        """
        t = dt.time()
    ...
    def _student_row_from_doc(self, doc) -> Optional[Tuple[str, str]]:
        """Return (student_id_str, name) from a Firestore student document."""
        data = doc.to_dict() or {}
        name = data.get('name')
        if name is None:
            return None
    ...
    def get_student_by_uid(self, nfc_uid: str) -> Optional[Tuple[str, str]]:
        ...
        Looks up by document ID (legacy) and by the nfc_uid field (imports / auto IDs).
        """
    ...
    def get_student_by_student_id(self, student_id: str) -> Optional[Tuple[str, str]]:
        ...
        try:
            ...
            # Document ID is often the school ID when nfc_uid was empty at creation
            if sid_key:
    ...
    def get_students_on_break(self) -> List[Tuple]:
        """Active bathroom breaks for this classroom: (student_uid, student_uid, name) for GUI compatibility."""
        try:
            breaks_ref = self.db.collection("bathroom_breaks")
            breaks_query = (
                breaks_ref
        ...
        try:
            ...
            for doc in water_query:
                try:
                    ...
                    print(
                        f"[FIREBASE AUTO-END] Ended water visit for {data['student_uid']} "
                        f"at period end {period_end_dt} (duration: {duration}min)"
                    )
